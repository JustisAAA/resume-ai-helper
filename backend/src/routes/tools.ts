...
try {
  ...
  console.error('[趋势预测] 加载Stack Overflow数据失败:', e);
}
...
// ========== 文件解析接口 ==========
router.post('/parse-file', authMiddleware, parseUpload.single('file'), async (req: Request, res: Response) => {
  try {
...
严格按JSON格式输出，不要包含markdown代码块标记。`;
    ...
    console.error('JD匹配失败:', error);
    res.status(500).json({ error: extractApiError(error, 'JD匹配失败') });
  }
});

// ========== 3. 面试题生成 ==========
router.post('/questions', authMiddleware, async (req: Request, res: Response) => {
  try {
    const userId = req.userId!;
...
// ========== 4. 求职攻略 ==========
router.post('/guide', authMiddleware, async (req: Request, res: Response) => {
  try {
    const userId = req.userId!;
...
请按你的角色设定执行求职攻略任务，用Markdown格式输出结果。`;
    ...
    console.error('求职攻略失败:', error);
    res.status(500).json({ error: extractApiError(error, '求职攻略失败') });
  }
});

// ========== 5. 岗位趋势预测分析（AI驱动） ==========
router.post('/trend', authMiddleware, async (req: Request, res: Response) => {
  try {
    const userId = req.userId!;
