  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 transition-colors">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {/* 头像 + 用户名卡片 */}
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200/60 dark:border-gray-700/60 shadow-sm p-8 mb-6 transition-colors relative overflow-hidden">
          {/* 背景装饰 */}
          <div className="absolute top-0 right-0 w-40 h-40 bg-gradient-to-bl from-indigo-100/40 dark:from-indigo-900/20 to-transparent rounded-full -mr-10 -mt-10 pointer-events-none" />
          <div className="absolute bottom-0 left-0 w-32 h-32 bg-gradient-to-tr from-purple-100/30 dark:from-purple-900/15 to-transparent rounded-full -ml-6 -mb-6 pointer-events-none" />
          <div className="flex items-center gap-6 relative">
            {/* 头像 */}
            <div className="relative flex-shrink-0">
              {/* 头像主体 */}
              <div className="relative w-24 h-24 rounded-full bg-gradient-to-br from-indigo-500 via-purple-600 to-indigo-700 flex items-center justify-center text-white text-3xl font-bold shadow-lg overflow-hidden ring-4 ring-white dark:ring-gray-900">
                {user?.avatar ? (
                  <img
              ...
              {/* 编辑按钮 */}
              <label className="absolute -bottom-1 -right-1 w-9 h-9 rounded-full bg-white dark:bg-gray-800 border-2 border-indigo-100 dark:border-gray-700 shadow-lg cursor-pointer flex items-center justify-center hover:bg-indigo-50 dark:hover:bg-gray-700 hover:border-indigo-300 dark:hover:border-indigo-500 transition-all group">
                <PencilIcon className="w-4 h-4 text-gray-500 dark:text-gray-400 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors" />
        ...
        {/* 设置列表 */}
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200/60 dark:border-gray-700/60 shadow-sm overflow-hidden transition-colors">
          ...
          <div className="flex items-center justify-between px-5 py-4">
            ...
            <button
              onClick={toggleTheme}
              className={`relative w-11 h-6 rounded-full transition-colors ${dark ? 'bg-indigo-500' : 'bg-gray-300'}`}
            >
      ...
      {showNameModal && (
        <div className="fixed inset-0 bg-black/40 dark:bg-black/60 flex items-center justify-center z-50" onClick={() => setShowNameModal(false)}>
          <div className="bg-white dark:bg-gray-900 rounded-2xl p-8 w-full max-w-md mx-4 border border-gray-200/60 dark:border-gray-700/60" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleChangeName} disabled={saving || !newName.trim()} className="flex-1 px-4 py-2.5 rounded-xl bg-indigo-600 text-white text-sm hover:bg-indigo-700 disabled:opacity-40 transition-colors">
                {saving ? '保存中...' : '保存'}
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/40 dark:bg-black/60 flex items-center justify-center z-50" onClick={() => setShowPwdModal(false)}>
          <div className="bg-white dark:bg-gray-900 rounded-2xl p-8 w-full max-w-md mx-4 border border-gray-200/60 dark:border-gray-700/60" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleChangePwd} disabled={saving || !pwdOld || !pwdNew || pwdNew !== pwdConfirm} className="flex-1 px-4 py-2.5 rounded-xl bg-indigo-600 text-white text-sm hover:bg-indigo-700 disabled:opacity-40 transition-colors">
                {saving ? '保存中...' : '保存'}
              </button>
            </div>
          </div>
