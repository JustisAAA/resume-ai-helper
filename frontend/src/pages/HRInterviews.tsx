...
const HRInterviews: React.FC = () => {
  ...
  const loadInterviews = async () => {
    try {
      setLoading(true);
      const res = await hrAPI.getInterviews(page);
          ...
          <div className="bg-white dark:bg-gray-800 shadow-sm rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-700/50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">候选人</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">职位</th>
                ...
                {interviews.map((interview) => (
                  <tr key={interview.id}>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        <div className="text-sm font-medium text-gray-900 dark:text-white">{interview.user.name}</div>
                        <div className="text-sm text-gray-500 dark:text-gray-400">{interview.user.email}</div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      {interview.status === 'COMPLETED' ? (
                        (interview.report || interview.feedback?.enterpriseEvaluation) ? (
                          <button
                            onClick={() => navigate(`/hr/interviews/${interview.id}/report`)}
                            className="inline-flex items-center text-brand-600 hover:text-brand-900 dark:text-brand-400 dark:hover:text-brand-200"
                          >
                        ...
                        <span className="text-gray-400 dark:text-gray-500">等待中</span>
                      )}
                    </td>
                  </tr>
                ))}
